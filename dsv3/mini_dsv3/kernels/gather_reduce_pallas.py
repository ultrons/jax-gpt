...
def sc_gather_reduce(
  ...
  if op.shape[0] % reduce_group_size != 0:
    raise ValueError(
        f"{op.shape[0]=} must be divisible by {reduce_group_size=}"
    )
  ...
  # Apr 28 nightly: pl.kernel kwarg renamed out_shape→out_type, _make_kernel
  # rewritten to use mpmd.mpmd_map (no more new_ref/freeze plumbing) — fixes
  # the lax.scan + grad ref-leak. Outer @jax.jit at train_step level handles
  # caching; we don't add an inner one here.
  @pl.kernel(
  ...
  def kernel(in_hbm_ref, idx_hbm_ref, weights_hbm_ref, out_hbm_ref):
    ...
    def idx_pipeline_body(idx_ref, *maybe_weights_ref):
      weights_ref = maybe_weights_ref[0] if maybe_weights_ref else None
      ...
      def data_pipeline_body(gather_ref, out_ref):
        ...
        subchunk_idx = idx_ref[row_slice]
        weights = (
            None
          ...
          for reduce_group in range(out_rows_per_step):
            ...
            for row_in_group in range(reduce_group_size):
              ...
              if packing == 1:
                ...
                assert packing == 2
                row_data = jnp.where(
                    lax.rem(subchunk_idx[row], 2) == 0, row_data[0], row_data[1]
                )
              if weights is not None:
                row_data *= weights[row]
                if topk_wgt_zero_nan:
                  row_data = jnp.where(
                      weights[row] == 0.0, jnp.zeros_like(row_data), row_data
                  )
              acc += row_data
            accs.append(acc)
          out = jnp.stack(accs, axis=0).astype(op.dtype)
          out_ref[:, pl.ds(col_base, unpack_col_chunk)] = out
      # TODO(apaszke): Add support for slicing single-sublane tiles in any dtype
    ...
    pltpu.emit_pipeline(
        idx_pipeline_body, grid=(num_row_chunks,), in_specs=in_specs
    )(idx_hbm_ref, *([weights_hbm_ref] if weights_hbm_ref is not None else []))
  return kernel(op, idx, topk_weights)
